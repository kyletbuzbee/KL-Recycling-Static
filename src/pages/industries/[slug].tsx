...
import { getIndustryPageData, getIndustries, urlFor } from "@/lib/sanity";
...
// Next.js 15 - params are now async in page components
interface PageProps {
  params: Promise<{ slug: string }>;
}

const IndustryPage: FC<IndustryPageProps & { params: { slug: string } }> = async ({ params }) => {
  ...
  try {
    const pageData = await getIndustryPageData(resolvedParams.slug);
    industry = pageData.industry;
    caseStudies = pageData.caseStudies || [];
    relatedServices = pageData.services || [];
  } catch (error) {
    console.warn('Failed to load industry from CMS, falling back to static data:', error);
  }

  // Fallback to static data
  if (!industry) {
    industry = getIndustryPage(resolvedParams.slug);
  }
  if (!industry) {
    return (
      <Layout>
        ...
        <div className="container mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative h-96">
              <Image src={industry.heroImage?.asset ? urlFor(industry.heroImage).url() : industry.heroImage} alt={`${industry.title} metal recycling`} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover rounded-xl" priority />
            </div>
          </div>
        </div>
      </section>
...
// Convert pages router to app router approach for Next.js 15
export async function generateStaticParams() {
  try {
    // Try CMS first
    const cmsIndustries = await require('@/lib/sanity').getIndustries();

    // Combine with static if CMS fails
    const staticIndustries = industryPages.map(ind => ({ slug: ind.slug }));

    // Get unique slugs
    const allSlugs = new Set([
      ...cmsIndustries.map((ind: any) => ind.slug?.current).filter(Boolean),
      ...staticIndustries.map(ind => ind.slug)
    ].filter(Boolean));

    return Array.from(allSlugs).map((slug: string) => ({ slug }));
  } catch (error) {
    console.warn('Failed to generate industry static params from CMS:', error);
    // Fallback to static
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  if (!industry) {
    return {
      title: 'Industry Not Found | K&L Recycling',
    };
  }

  return {
    title: `${industry.title} Metal Recycling Services | K&L Recycling`,
    description: `Specialized ${industry.title.toLowerCase()} scrap metal recycling solutions in Texas. ${industry.description} Book a consultation today for competitive pricing and expert service.`,
    keywords: industry.seoKeywords?.join(", "),
    canonical: `https://www.klrecycling.com/industries/${industry.slug}`,
  };
}
