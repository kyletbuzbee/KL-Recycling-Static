...
const AnimatedBanner: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prev: number) => (prev + 1) % bannerItems.length);
    ...
    <div className="relative bg-gradient-to-r from-royal-blue-600 via-electric-blue-600 to-royal-blue-700 text-white overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 py-8">
        <div className="flex items-center justify-center gap-8 max-w-6xl mx-auto">
          ...
          <div className="hidden md:flex flex-shrink-0">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300">
              <img src="/assets/certifications-logo/isn-logo.png" alt="ISN Certified" width={120} height={60} />
            </div>
          </div>
          ...
          <div className="flex-1 max-w-2xl mx-4">
            <AnimatePresence mode="wait">
              <motion.div key={currentIndex} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="text-center">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.3 }} className="text-base md:text-lg opacity-90">
          ...
          {/* Right Side - REMA Certification Logo */}
          <div className="hidden md:flex flex-shrink-0">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 hover:bg-white/20 transition-all duration-300">
              <img src="/assets/certifications-logo/rema-certified-logo.png" alt="REMA Certified" width={120} height={60} />
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-center space-x-2 mt-6">
          {bannerItems.map((_: BannerItem, index: number) => (
