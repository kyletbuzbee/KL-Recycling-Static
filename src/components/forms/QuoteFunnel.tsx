...
import FileUpload from "@/features/contact/components/FileUpload";
...
const QUOTE_FORM_KEY = 'kl_quote_form_data';
const QUOTE_FORM_STEP_KEY = 'kl_quote_form_step';
const QUOTE_FORM_TIMESTAMP_KEY = 'kl_quote_form_timestamp';

// Expire saved data after 24 hours
...
const QuoteFunnel: FC<QuoteFunnelProps> = ({ onClose, initialService }) => {
  ...
  // Extended steps with photo upload
  const steps = [
    ...
    { id: "photos", title: "Upload Photos", description: "Show us your project" },
  ...
  useEffect(() => {
    try {
      const savedTimestamp = localStorage.getItem(QUOTE_FORM_TIMESTAMP_KEY);
      const now = Date.now();

      if (savedTimestamp && (now - parseInt(savedTimestamp)) > EXPIRY_TIME) {
        // Clear expired data
        localStorage.removeItem(QUOTE_FORM_KEY);
        localStorage.removeItem(QUOTE_FORM_STEP_KEY);
        localStorage.removeItem(QUOTE_FORM_TIMESTAMP_KEY);
      } else {
        // Load saved data
        const savedData = localStorage.getItem(QUOTE_FORM_KEY);
        const savedStep = localStorage.getItem(QUOTE_FORM_STEP_KEY);

        if (savedData) {
          const parsedData = JSON.parse(savedData);
          setFormData(parsedData);
          if (parsedData.serviceType) {
            setSelectedService(parsedData.serviceType);
          }
        }
  ...
  useEffect(() => {
    if (Object.keys(formData).length > 0) {
      try {
        localStorage.setItem(QUOTE_FORM_KEY, JSON.stringify(formData));
        localStorage.setItem(QUOTE_FORM_STEP_KEY, currentStep.toString());
        localStorage.setItem(QUOTE_FORM_TIMESTAMP_KEY, Date.now().toString());
      } catch (error) {
  ...
  const renderPhotoUploadStep = () => (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h3 className="text-2xl font-bold text-gray-900 mb-2">Upload Photos of Your Project</h3>
        <p className="text-gray-600">Help us provide the most accurate quote by showing us your scrap metal or project.</p>
      </div>
      ...
      <div className="bg-blue-50 p-4 rounded-lg">
        <h4 className="font-semibold text-blue-900 mb-2">📸 Why upload photos?</h4>
        <ul className="text-blue-800 text-sm space-y-1">
          <li>• Get more accurate material assessments</li>
          <li>• Receive specialized handling recommendations</li>
          <li>• Faster quote turnaround time</li>
          <li>• Better understanding of project scope</li>
        </ul>
      </div>
      ...
      <div className="bg-green-50 p-4 rounded-lg">
        ...
        <ul className="text-green-800 text-sm space-y-1">
          <li>• Show different angles of your scrap pile</li>
          <li>• Include any hazardous materials labeling</li>
          <li>• Capture the total volume when possible</li>
          <li>• Document condition of materials</li>
        </ul>
      </div>
    </div>
  );
